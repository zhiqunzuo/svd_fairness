...
class FairBatch(Sampler):
    ...
    def adjust_lambda(self):
        ...
        if self.fairness_type == 'eqopp':
            ...
            eo_loss = criterion ((F.tanh(logit.squeeze(1))+1)/2, (self.y_data+1)/2)
            ...
            dp_loss = criterion((F.tanh(logit.squeeze(1))+1)/2, ones_tensor) # Note that ones tensor puts as the true label
